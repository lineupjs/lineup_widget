...
export function pushRanking(data: LocalDataProvider, ranking: ILineUpRanking): Ranking {
  ...
  (typeof ranking.sort_by === 'string' ? [ranking.sort_by] : ranking.sort_by || []).forEach((s) => r.sortBy(s));
  (typeof ranking.group_by === 'string' ? [ranking.group_by] : ranking.group_by || []).forEach((s) => r.groupBy(s));
  return r.build(data);
}
